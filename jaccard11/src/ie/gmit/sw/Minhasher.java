...
public class Minhasher {
	 ...
	 * @return
	 */
	public double jaccardIndex(final Set<Integer> s1, final Set<Integer> s2) {
	...
	/**
	 * Compute the exact jaccard index between two sets, represented as arrays
	 * of booleans.
	 * 
	 * @param s1
	 * @param s2
	...
	/**
	 * Dictionary size (is also the size of vectors if the sets are provided as
	 * vectors).
	 */
	 ...
	 *
	 * @param size
	 *            the number of hash functions (and the size of resulting
	 *            signatures)
	 * @param dict_size
	 */
	public Minhasher(final int size, final int dict_size) {
	 ...
	 * @param dict_size
	 */
	public Minhasher(final double error, final int dict_size) {
	 ...
	 * @param dict_size
	 * @param seed
	 *            random number generator seed. using the same value will
	 *            guarantee identical hashes across object instantiations
	 */
	public Minhasher(final int size, final int dict_size, final long seed) {
	 ...
	 *            guarantee identical hashes across object instantiations
	 */
	public Minhasher(final double error, final int dict_size, final long seed) {
	 ...
	 * @param sig2
	 *            MinHash signature of set2 (produced using the same
	 *            coefficients)
	...
	private int h(final int i, final int x) {
		return (int) ((hash_coefs[i][0] * (long) x + hash_coefs[i][1]) % dict_size);
	}
