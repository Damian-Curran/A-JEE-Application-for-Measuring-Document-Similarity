...
public class Processor {
	...
	public void checkQueue() throws IOException, TimeoutException {
		...
		Consumer consumer = new DefaultConsumer(channel) {
			...
			public void handleDelivery(String consumerTag, Envelope envelope, AMQP.BasicProperties properties,
				...
				// manipulate AMQP basic properties to get message id stored and
				// correlation id
				...
				t = new Thread(replyProps.getCorrelationId()) {
					public void run() {
						...
						s = new Shinglizer();
						// instantiates a set of integer for storing shingles
						// returned
						Set<Integer> shingles = null;
						try {
							// pass message into shingles and returns sets of
							// shingles size 3
						...
						System.out.println(
								"Real similarity (Jaccard index)" + minhash.jaccardIndex(shingles, shingles));

						// sets document d shingle and title
						d.setShingle(shingles);
						d.setTitles(replyProps.getMessageId());

						// sets boolean to use later to check if map contains
						// needed task
						boolean error = true;
						...
						for (Document des : docs) {
							// puts each title and jaccardIndex(once computed)
							// into the map
							results.put(des.getTitles(), minhash.jaccardIndex(shingles, des.getShingle()) * 100);
						}

						// try/catch on calling result queue
						// which puts the finished task to an outward queue to
						// show user
						try {
	...
	public void resultQueue(Map<String, Object> results, String taskId) throws IOException, TimeoutException {
		...
		// sets props correlationId to be that of the ID received from the
		// publisher(clientServlet)
		AMQP.BasicProperties props = new AMQP.BasicProperties.Builder().correlationId(taskNumber).headers(results)
				.build();
